...
const LoadingResult = ({step, message,back,color,plateform}) => (
  <div className="container empty">
    <div className="empty-icon">
      <img src={image} className="h-8 mb-4" alt="" />
    </div>
    <p className="empty-title h3">{step}<span className={`bg-${back} text-${color}`}>{plateform}</span> </p>
    {/* <p className="empty-title h3">{message} </p> */}
